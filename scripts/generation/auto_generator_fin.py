...
### 核心任务：
1. **身份确认**：根据提供的化学式和空间群信息，建立材料的基本物理图像。
2. **数据锚定 (Data Anchoring)**：深入数据细节，锁定 1-3 个具有极高科研价值的**定量锚点**。
...
### 核心约束：
1. **直接输出**：不要废话，**直接从一级标题（材料名称/化学式）开始**。
...
def process_single_file(json_filename):
    """
    处理单个文件：直接读取元数据 -> Prompt1 -> Prompt2 -> 保存
    (已修复格式化字符串错误)
    """
    ...
    try:
        ...
        # 1. 基础元数据
        # 即使读不到，也给个默认字符串，防止 format 报错
        formula = str(data.get('formula_pretty', "Unknown"))
        
        # 安全处理 symmetry (防止它不是 dict)
        symmetry = data.get('symmetry')
        if not isinstance(symmetry, dict):
            symmetry = {}
        sym_symbol = str(symmetry.get('symbol', 'N/A'))
        sym_number = str(symmetry.get('number', 'N/A'))
        
        # 2. 物理参数 (强制类型转换，处理 None)
        ...
        efermi = safe_float(data.get('efermi'))
        band_gap = safe_float(data.get('band_gap'))
        
        # is_metal 有时候是 bool，有时候可能是 None
        is_metal = bool(data.get('is_metal', False))
        
        nb_bands = data.get('nb_bands', 0)
        labels = list(data.get('labels_dict', {}).keys())
        
        # 4. 构建精简的 Context 给 LLM
        # 现在 efermi 和 band_gap 肯定是 float，:.4f 绝对安全
        llm_context = f"""
        【材料物理特征摘要 (From Materials Project Metadata)】
        - Material ID: {mp_id}
        - Chemical Formula: {formula}
        - Symmetry: {sym_symbol} (Space Group #{sym_number})
        - Fermi Energy: {efermi:.4f} eV
        - Is Metal: {is_metal}
        - Band Gap: {band_gap:.4f} eV (Calculated)
        ...
        # 5. 附加少量能带采样数据
        sample_bands = []
        raw_spin_up = data.get('spin_up')
        
        # 只有当 raw_spin_up 是列表时才处理
        if isinstance(raw_spin_up, list) and len(raw_spin_up) > 0:
             # data['spin_up'] 结构通常是 [band_index][k_point_index]
             # 这里简单取前 3 条能带的前 10 个数据点作为示例
             for i in range(min(3, len(raw_spin_up))):
                 # 再次检查内层是否为 list
                 if isinstance(raw_spin_up[i], list):
                    sample_bands.append(raw_spin_up[i][:10])
        
        llm_context += f"\n- Sample Band Data (Spin Up, First 3 Bands, First 10 K-points): {sample_bands}"
        
        print(f"  -> [OK] 特征提取成功: {formula} ({sym_symbol}), Gap={band_gap:.2f}eV")

    except Exception as e:
        # 打印更详细的堆栈信息方便排查，但为了生产环境整洁，这里只打 ERROR
        import traceback
        traceback.print_exc()
        print(f"  -> [WARN] 数据读取异常 ({e})，该文件可能不完整。")
        return False

    # 2. 初始化对话
    history = [{'role': 'system', 'content': '你是一个计算材料学专家。'}]

    # 3. Round 1: 分析
    print("  -> 发送 Prompt 1 (分析)...")
